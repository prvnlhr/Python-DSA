...
# Two pointer concept Ref and main pointer
# first move the ref pointer n steps ahead
# then , move the ref pointer to end and main pointer from start
# when ref reaches end main reaches nth node
def findNthNodeFromLast(head, n):
    ref = head
    main = head
    count = 0
    # IMP:: count < n , if count <= n will will get wrong answer
    while count < n:
        count = count + 1
        ref = ref.next
    while ref:
